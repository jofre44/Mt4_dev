...
class Pytrader_API:
    ...
    def Get_last_x_ticks_from_now(self,
        ...
        dt = np.dtype(
            [('date', np.int64), ('ask', np.float64), ('bid', np.float64), ('last', np.float64), ('volume', np.int32)])
        ...
        if (self.nbrofticks > self.max_ticks):
            ...
            for index in range(0, iloop):
                self.command = 'F021#3#' + self.get_broker_instrument_name(
                    self.instrument_name_universal) + '#' + str(index * self.max_ticks) + ':' + str(
                    self.max_ticks) + '#'
                ok, dataString = self.send_command(self.command)
                if not ok:
                    self.command_OK = False
            ...
            self.command = 'F021#3#' + self.get_broker_instrument_name(self.instrument_name_universal) + '#' + str(
                0) + '#' + str(self.nbrofticks) + '#'
            ok, dataString = self.send_command(self.command)
    ...
    def Get_all_orders(self) -> pd.DataFrame:
        ...
        for value in range(0, len(x)):
            ...
            rowOrders = pd.Series({
                'ticket': int(y[0]), 'instrument': self.get_universal_instrument_name(str(y[1])),
                'order_type': str(y[2]), 'magic_number': int(y[3]),
                'volume': float(y[4]), 'open_price': float(y[5]), 'stop_loss': float(y[6]), 'take_profit': float(y[7]),
                'comment': str(y[8])})
    ...
    def Get_all_open_positions(self) -> pd.DataFrame:
        ...
        for value in range(0, len(x)):
            ...
            rowPositions = pd.Series(
                {
                    'ticket': int(y[0]), 'instrument': self.get_universal_instrument_name((y[1])),
                    'position_type': str(y[2]),
    ...
    def Get_all_closed_positions(self,
        ...
        for value in range(0, len(x)):
            ...
            rowClosedPositions = pd.Series(
                {
                    'position_ticket': int(y[0]), 'instrument': self.get_universal_instrument_name(str(y[1])),
                    'order_ticket': int(y[2]),
                    'position_type': str(y[3]), 'magic_number': int(y[4]), 'volume': float(y[5]),
    ...
    def Open_order(self,
        ...
        comment.replace('$', '')
        comment.replace('!', '')
        self.command = 'F070#8#' + self.get_broker_instrument_name(
            self.instrument_name_universal) + '#' + ordertype + '#' + str(volume) + '#' + \
                       str(openprice) + '#' + str(slippage) + '#' + str(magicnumber) + '#' + str(stoploss) + '#' + str(
            takeprofit) + '#' + str(comment) + '#'
        ok, dataString = self.send_command(self.command)
        if not ok:
            self.command_OK = False
            return int(-1)
